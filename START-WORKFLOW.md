...
**Meme beat (KLIPY):** the owner supplied a KLIPY API key for meme/GIF content as a short humorous beat. **Do not composite KLIPY media into a rendered video yet.** KLIPY's terms require media to load directly from their URLs, forbid re-hosting, and expect client-side requests — our pipeline bakes media into an MP4 and re-hosts it, which needs written approval from developers@klipy.com first. Separately, meme rights for commercial republication are unresolved. Until both are settled, use `python tools/klipy.py search "..."` for **inspiration only** and express the joke as an original Stiles illustration. The meme endpoints are also not enabled for the current key. See KLIPY.md.
