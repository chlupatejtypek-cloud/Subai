...
**Meme beat (KLIPY / GIPHY):** the owner supplied both a KLIPY and a GIPHY API key for meme content as a short humorous beat. **Do not composite media from either provider into a rendered video.** GIPHY's terms licence content for *"personal and non-commercial purposes"* only and licence the API solely for apps that interface with GIPHY; KLIPY forbids re-hosting and server-side fetching without written approval. Our pipeline bakes media into an MP4 and re-hosts it, so both need written permission first, and meme rights for commercial republication are separately unresolved. KLIPY's meme endpoints are also not enabled for the current key. Until that is settled, use `python tools/klipy.py search "..."` or `python tools/giphy.py search "..."` for **inspiration only**, then express the joke as an original Stiles illustration — which also keeps the channel's visual identity consistent. See MEME-APIS.md.
