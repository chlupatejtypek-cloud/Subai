...
- Generate **10 original images with the agent image tool, then attempt 5 additional Agnes AI stills** (owner clarification 2026-09-07). Target 15 accepted images; registry budget 15. Agnes still endpoint is now verified: `tools/agnes-image.py`, `agnes-image-2.0-flash`, reference images in `extra_body.image`. Five requests succeeded in the awkward-silence production. Disclose any future failure/rejection; do not confuse still generation with image-to-video. Rich foreground/midground/background; every edge full bleed. No white/cream matte, side strip, border or lower panel. Never prompt for a caption-safe lane.
