...
files=[f for f in p.rglob('*') if f.is_file() and not set(f.relative_to(p).parts)&{'output','__pycache__'} and f.name not in {'assets.json','cloudinary.md','source-manifest.json','.media-keep','cleanup.md'} and f.relative_to(p).as_posix() not in {'audio/final-mix.wav','audio/sfx.wav','assets/priced-hook-native.mp4'}]
...
raw=buf.getvalue();count=math.ceil(len(raw)/4000000);final=p/'output/anchoring-remake.mp4';uploads=[final]
...
results=[];folder='subai/productions/stiles-psychology/2026-09-08-anchoring/remake'
for f in uploads:
