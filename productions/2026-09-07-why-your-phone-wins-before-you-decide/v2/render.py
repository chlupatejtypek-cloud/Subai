...
def sfx():
 ...
 for start,f,d,amp in [(SCENES[4][1]+.65,1050,.12,.014),(SCENES[7][1]+.55,210,.15,.018)]:
...
if __name__=='__main__':
 ...
 run(['ffmpeg','-y','-v','error','-i',str(O/'silent.mp4'),'-i',str(P/'audio/narration-v2.wav'),'-i',str(P/'audio/sfx.wav'),'-filter_complex',f"[0:v]ass={P/'captions.ass'}[v];[1:a]loudnorm=I=-16:TP=-2.0:LRA=9[voice];[voice][2:a]amix=inputs=2:normalize=0,alimiter=limit=0.94:level=false[a]",'-map','[v]','-map','[a]','-t',str(DURATION),'-c:v','libx264','-preset','fast','-crf','19','-threads','2','-pix_fmt','yuv420p','-c:a','aac','-b:a','192k','-ar','48000','-movflags','+faststart',str(O/'phone-checking-v2.mp4')]);print('FINAL_READY',flush=True)
