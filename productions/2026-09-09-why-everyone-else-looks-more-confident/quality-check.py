...
p=Path(__file__).resolve().parent;final=p/'output/liking-gap-final.mp4';d=json.loads(subprocess.check_output(['ffprobe','-v','error','-show_format','-show_streams','-of','json',str(final)]));v=next(x for x in d['streams'] if x['codec_type']=='video');a=next(x for x in d['streams'] if x['codec_type']=='audio');duration=float(d['format']['duration']);assert 40<=duration<=60;assert (v['width'],v['height'],v['r_frame_rate'])==(1080,1920,'30/1');assert int(v['nb_frames'])==1503
lines=[x for x in (p/'captions.ass').read_text().splitlines() if x.startswith('Dialogue:')];ts=lambda x:sum(float(v)*k for v,k in zip(x.split(':'),(3600,60,1)));caps=[(ts(x.split(',')[1]),ts(x.split(',')[2])) for x in lines];assert len(caps)==167 and all(a<b for a,b in caps) and all(caps[i][1]<=caps[i+1][0] for i in range(len(caps)-1));assert caps[-1][1]<=duration
maxwidth=0
...
sheet.save('/home/user/liking-gap-final-qc.jpg')
