...
while time.time()<deadline:
 r=requests.get(base+'/actions/workflows/publish-calendar.yml/runs',headers=headers,params={'per_page':15},timeout=30);r.raise_for_status();runs=[x for x in r.json()['workflow_runs'] if x['event']=='workflow_dispatch' and x['head_sha']==json.loads((p/'publication-request.json').read_text())['head_sha']]
 if runs:
...
r=requests.get(base+'/contents/calendar/2026-09-07_2026-10-06.json',headers=headers,params={'ref':'main'},timeout=30);r.raise_for_status();cal=json.loads(base64.b64decode(r.json()['content']));item=next(x for x in cal['items'] if x['id']=='SP-20260908-3');assert item['status']=='scheduled',item['status'];print('Calendar',item['status'],item.get('youtube_video_id'),flush=True)
...
for _ in range(15):
 ...
 time.sleep(10)
assert video['status']['privacyStatus']=='private';assert video['status']['publishAt']=='2026-09-08T21:00:00Z';assert video['processingDetails']['processingStatus']=='succeeded'
