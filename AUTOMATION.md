...
- `qa.reviewed_by`, ISO `qa.reviewed_at`, `qa.source_image_count` 1–15 (current owner budget: 10 agent stills + 5 attempted Agnes stills), `qa.voice_provider: fish_audio`, `qa.voice_reference_id: fb7ec16ca51a45a5a4db881244d7990a`.
...
Write the entire script as one flowing paragraph and make one Fish synthesis request, not one request per sentence. Keep accepted 1.06 post-edit tempo; preserve normal pauses. Ten agent-generated images plus five explicitly attempted Agnes AI stills, not five unattempted optional extras. `tools/agnes-image.py` uses the verified image endpoint, not image-to-video. Five requests succeeded for SP-20260907-2; no claim this establishes unlimited future free quota. Original quiet scene-motivated procedural SFX are permitted; no music bed.
